...
class NumberPronunciationRuntime extends TaskRuntimeBase {
  ...
  Future<void> _handleSpeechResult(
    ...
    final resolvedWords = _resolveRecognizedText(
      finalText: recognizedWords,
      partialText: _lastPartialResult,
    );
    _lastPartialResult = '';
    await _handleAttemptResult(recognizedText: resolvedWords);
  }

  String _resolveRecognizedText({
    required String finalText,
    required String partialText,
  }) {
    final trimmedFinal = finalText.trim();
    final trimmedPartial = partialText.trim();
    ...
    final partialAccepted = _answerMatcher.isAcceptedAnswer(trimmedPartial);
    final finalAccepted = _answerMatcher.isAcceptedAnswer(trimmedFinal);
    if (partialAccepted && !finalAccepted) {
      _log(
        'Speech using partial instead of weaker final: '
        '"$trimmedPartial" > "$trimmedFinal"',
      );
      return partialText;
    }
    if (finalAccepted) {
    ...
    final partialPreview = _answerMatcher.previewRecognition(
      trimmedPartial.replaceAll(',', ''),
    );
    final finalPreview = _answerMatcher.previewRecognition(
      trimmedFinal.replaceAll(',', ''),
    );
    final partialMatchCount = partialPreview.matchedSegmentIndices.length;
    final finalMatchCount = finalPreview.matchedSegmentIndices.length;
    final partialHasBetterMatch = partialMatchCount > finalMatchCount;
    final partialHasRicherEquivalentMatch =
        partialMatchCount == finalMatchCount &&
        partialMatchCount > 0 &&
        partialPreview.recognizedTokens.length >
            finalPreview.recognizedTokens.length;

    if (partialHasBetterMatch || partialHasRicherEquivalentMatch) {
      _log(
        'Speech using partial instead of weaker final: '
        '"$trimmedPartial" > "$trimmedFinal"',
      );
  ...
  Future<void> _schedulePartialFastAcceptIfEligible({
    ...
    _log(
      'Speech partial fast-accept committed immediately: '
