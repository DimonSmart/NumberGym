...
List<TrainingItemId> buildNumberCardIds() {
  ...
  for (var i = 0; i <= 9; i++) {
    ids.add(TrainingItemId(type: TrainingItemType.digits, number: i));
  }
  for (var i = 10; i <= 99; i++) {
    ids.add(TrainingItemId(type: TrainingItemType.base, number: i));
  }
  for (var i = 100; i <= 900; i += 100) {
    ids.add(TrainingItemId(type: TrainingItemType.hundreds, number: i));
  }
  for (var i = 1000; i <= 9000; i += 1000) {
    ids.add(TrainingItemId(type: TrainingItemType.thousands, number: i));
  }
  return ids;
}
...
List<NumberPronunciationTask> buildNumberCards({
  ...
  return ids.map((id) {
    ...
    return NumberPronunciationTask(
      ...
      prompt: prompt,
      language: language,
      answers: <String>[converter(value), prompt],
    );
  }).toList();
}
