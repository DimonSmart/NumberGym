...
void main() {
  ...
  test('legacy debug forced method key is ignored', () async {
    ...
    expect(repository.readDebugForcedMode(), isNull);
  });

  test('new debug forced mode key is used', () async {
    final repository = SettingsRepository(box);
    await box.put(debugForcedModeKey, 'speak');

    expect(repository.readDebugForcedMode(), 'speak');
  });
